...
path = "/network/aopp/chaos/pred/kloewer/julsdata/forecast2/nx400comm/"
time_step = 201
...
y2 = y[1:100]
lower_half = CartesianIndices((1:(length(y)÷2),1:length(x)))
...
axs[2,1].contour(x,y2,abs.(sst2'-sst1')[lower_half],levs1,colors="C3",linewidths=lw)
axs[2,1].contourf(x,y2,abs.(sst2'-sst1')[lower_half],levs2,colors="C3",alpha=α)

axs[3,1].contour(x,y2,abs.(sst3'-sst1')[lower_half],levs1,colors="C3",linewidths=lw)
axs[3,1].contourf(x,y2,abs.(sst3'-sst1')[lower_half],levs2,colors="C3",alpha=α)

axs[1,2].contourf(x,y,sst4',levs,cmap=cmap)
axs[2,2].contourf(x,y,sst5',levs,cmap=cmap)
axs[3,2].contourf(x,y,sst6',levs,cmap=cmap)

axs[1,2].contour(x,y2,abs.(sst4'-sst1')[lower_half],levs1,colors="C3",linewidths=lw)
axs[1,2].contourf(x,y2,abs.(sst4'-sst1')[lower_half],levs2,colors="C3",alpha=α)

axs[2,2].contour(x,y2,abs.(sst5'-sst1')[lower_half],levs1,colors="C3",linewidths=lw)
axs[2,2].contourf(x,y2,abs.(sst5'-sst1')[lower_half],levs2,colors="C3",alpha=α)

axs[3,2].contour(x,y2,abs.(sst6'-sst1')[lower_half],levs1,colors="C3",linewidths=lw)
axs[3,2].contourf(x,y2,abs.(sst6'-sst1')[lower_half],levs2,colors="C3",alpha=α)
...
axs[2,1].set_title("Communication: Float16",loc="left",fontsize=8)
axs[3,1].set_title("Communication: BFloat16",loc="left",fontsize=8)

axs[1,2].set_title("Communication: Posit(16,1)",loc="left",fontsize=8)
axs[2,2].set_title("Communication: Posit(16,2)",loc="left",fontsize=8)
axs[3,2].set_title("Communication: Posit(8,0)",loc="left",fontsize=8)
...
savefig("plots/snapshot_comm.png",dpi=300)
close(fig)
