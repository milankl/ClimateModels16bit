...
\subsection{Re-ordering arithmetic operations in the shallow water system}
...
To avoid division and subsequent multiplication with large numbers in the dissipative terms (Eq. \ref{eq:diss}) throughout the numerical model integration, we rescale $\mathbf{D}$  accordingly
\begin{equation}
...
The semi-Lagrangian advection scheme is based on the idea to solve the advection equation with respect to its Lagrangian formulation. In the absence of sources and sinks, the Lagrangian point-of-view states that the tracer concentration $q$ does not change following its trajectory. The concentration $q$ at a departure point $\mathbf{x}_d$ is therefore the same as the concentration at the arrival point $\mathbf{x}_a$ later, which coincide with the grid points. Based on the flow velocity at the arrival point, a departure point $\mathbf{x}_d$ is derived. In order to avoid large numbers of the coordinates ($L_x = 2 \cdot 10^6$m), non-dimensional departure points $\mathbf{\tilde{x}}_{d,rel}$ relative to the arrival point are computed as
\begin{equation}
\mathbf{\tilde{x}}_{d,rel} = - \mathbf{u}(\mathbf{x}_a,t+\Delta t_{\op{adv}}) \frac{\Delta t_{\op{adv}}}{\Delta}.
\label{eq:relcoord}
\end{equation}
where $t$ denotes the previous and $t+\Delta t_{\op{adv}} the current time step. A scaling with the grid-spacing $\Delta$ is applied to obtain an equation with all terms being $\mathcal{O}(1)$ and therefore representable with 16bit arithmetics. In practice, when converting the relative departure point $\mathbf{\tilde{x}}_{d,rel}$ to an array index, the floor function is used in combination with integer arithmetics, which can be performed without rounding errors. This essentially separates a computation with reals into two parts. One that can be computed with integers without rounding errors, and a calculation with reals, with a removed offset to avoid rounding errors.
