...
\title{Posit numbers as an alternative to floating-point numbers for weather and climate models}
...
\begin{keypoints}
\item Posit numbers have smaller arithmetic rounding errors compared to floating-point numbers in weather and climate applications
\item Reduced rounding errors with posits enable reliable shallow water simulations at 16bit
\item Mixed 16bit/32bit precision is recommended for weather and climate models on available floating-point based hardware.
\end{keypoints}
...
%Posit numbers extend the IEEE standard on floating-point arithmetic \cite{IEEE} by introducing regime bits, that are mostly responsible for the dynamic range of representable numbers. Instead of having a fixed length, regime bits are defined as the leading identical bits after the sign bit. The flexible length allows the significand (or mantissa) to occupy more bits when less regime bits are needed, which is the case for numbers around one. A resulting higher precision around one is traded against a gradually lower precision for very large or very small numbers. This is in contrast to floating-point numbers, that have a fixed number of significant bits (16-bit half precision floats have 1 sign, 5 exponent and 10 significant bits) and consequently a constant number of significant digits throughout their dynamic range of representable numbers. 
...
julia> a = Posit16(12.3) 	# convert float to 16bit posit
julia> c = a+a            # posit addition 
julia> bits(c," ")        # bits split in sign, regime, exponent and fraction
"0 1110 0 1000100110"
julia> Float64(c)         # convert back to Float64
24.59375
\end{verbatim}
\caption{Example use of the posit emulator \emph{SoftPosit.jl} in the Julia shell.}
\label{fig:emulator}
\end{figure}

In order to use posits on a conventional CPU we use the posit emulator \emph{SigmoidNumbers} written by Isaac Yonemoto in Julia \cite{Bezanson2014}. This emulator defines conversion to and from floats and arithmetic operations with posits (see Fig. \ref{fig:emulator} for an example). Consequently, posit arithmetic can be used for the numerical integration of the Lorenz equations (section \ref{sec:L63}) and the shallow water model (section \ref{sec:swm}).
...
\begin{figure}
\includegraphics[width=1\textwidth]{figs/decimal_precision_hist.png}
\caption{(a) Decimal precision of various 16-bit number formats. Dashed vertical lines indicate the dynamic range of representable numbers for each format. (b) Histogram of results of all arithmetic operations in the rescaled Lorenz system, that are subject to rounding errors, considering absolute values.}
\label{fig:dec_acc}
\end{figure}
...
\begin{figure}
\includegraphics[width=1\textwidth]{figs/lorenz_attractor.png}
\caption{The Lorenz attractor computed with different arithmetics and precision. (a) 64bit double precision floats, (b) 16-bit half precision floats, (c) 16-bit posits with 1 exponent bit and (d) 16-bit integers. The scaling of the Lorenz equations (Eq. \ref{eq:L63s}) is (a,b) $s=1$, (c) $s=0.1$ and (d) $s=100$. All curves are integrated for the same number of time-steps.}
\label{fig:L63}
\end{figure}
...
\section{Fluid circulation model}
\label{sec:swm}
...
\begin{figure}
\includegraphics[width=1\textwidth]{figs/snapshot_tracer.png}
\caption{Snapshot of tracer concentration simulated by the shallow water model, based on (a) double precision floats and (b) posit arithmetic (16 bit with 2 exponent bits) and (c) half precision floats. The tracer was injected uniformly in the left half of the domain 25 days before. This simulation was run at a resolution of $\Delta = 10$km (200x100 grid points). The corresponding video can be found at \url{http://milank.de/videos/swm_posit_tracer.mp4}}
\label{fig:snapshot}
\end{figure}
...
\begin{figure}
\includegraphics[width=1\textwidth]{figs/forecast_error.pdf}
\caption{Forecast error measured as the root mean square error (RMSE) of sea surface height taking the double precision forecast as reference. The RMSE is normalised by a mean forecast error at very long lead times. Solid lines represent the median of 280 forecasts per number format. The shaded areas denote the interquartile range.}
\label{fig:rmse}
\end{figure}
...
\acknowledgments
 The authors would like to thank Isaac Yonemoto for providing the Julia-based emulator for posit numbers, that was used for this study.
